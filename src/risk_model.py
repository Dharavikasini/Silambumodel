...
def calculate_location_features(
    ...
    df["distance_from_normal_baseline"] = df["distance_from_baseline_location"].copy()
    df["maximum_distance_from_normal_baseline"] = df["distance_from_normal_baseline"].cummax()
    return df
...
def create_risk_labels(df: pd.DataFrame) -> np.ndarray:
    ...
    if missing:
        raise ValueError(f"Scenario-to-risk mapping missing for {int(missing)} rows.")
...
def create_model_features(
    ...
    df["mean_acceleration_magnitude"] = df["acceleration_magnitude"]
    df["std_acceleration_magnitude"] = 0.0
    df["max_acceleration_magnitude"] = df["acceleration_magnitude"]
    df["mean_gyroscope_magnitude"] = df["gyroscope_magnitude"]
    df["std_gyroscope_magnitude"] = 0.0
    df["max_gyroscope_magnitude"] = df["gyroscope_magnitude"]
    ...
    df["mean_absolute_heart_rate_change"] = df["heart_rate_change"]
    df["mean_speed"] = df["speed"]
    df["std_speed"] = 0.0
    df["max_speed"] = df["speed"]
    df["speed_range"] = 0.0
    df["mean_absolute_speed_change"] = df["speed_change"]
    df["latitude_displacement"] = df["latitude_delta"]
    df["longitude_displacement"] = df["longitude_delta"]
    df["total_approximate_displacement"] = np.hypot(df["latitude_delta"], df["longitude_delta"])
    df["mean_spo2"] = df["spo2"]
    df["min_spo2"] = df["spo2"]
    df["tamper_count"] = df["tamper"].astype(int)
    df["sos_count"] = df["sos"].astype(int)

    expected_columns = list(anomaly_scaler.feature_names_in_)
    print("Anomaly scaler expects:", expected_columns)
    print("Provided anomaly features:", list(df.columns))

    missing = [column for column in expected_columns if column not in df.columns]
    if missing:
        raise ValueError(f"Missing anomaly scaler features: {missing}")

    features = df[expected_columns].copy()
    if list(features.columns) != list(expected_columns):
        raise ValueError(
            "Anomaly feature column order mismatch. "
            f"Expected: {expected_columns}. Provided: {list(features.columns)}"
        )

    features = features[expected_columns]
    anomaly_scaled = anomaly_scaler.transform(features)
    df["isolation_forest_anomaly_score"] = anomaly_model.decision_function(anomaly_scaled)

    df["activity_encoded"] = df["activity"].map(activity_map).astype(int)
...
def prepare_data() -> tuple[pd.DataFrame, np.ndarray, pd.DataFrame, np.ndarray, dict[str, object]]:
    """Create chronologically ordered train/test splits using the risk feature set."""
    df = load_data()
    df = df.copy()
    df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")
    ...
    with ANOMALY_MODEL_PATH.open("rb") as anomaly_model_file:
        anomaly_model = pickle.load(anomaly_model_file)
    with ANOMALY_SCALER_PATH.open("rb") as anomaly_scaler_file:
        anomaly_scaler = pickle.load(anomaly_scaler_file)
    ...
    normal_training_scenarios = {"normal_rest", "normal_walking", "normal_running"}
    normal_train_df = train_df[train_df["scenario"].isin(normal_training_scenarios)].copy()
    if normal_train_df.empty:
        raise ValueError("No normal training rows are available for the baseline calculation.")

    baseline_latitude = float(normal_train_df["latitude"].mean())
    baseline_longitude = float(normal_train_df["longitude"].mean())

    activity_values = sorted(df["activity"].dropna().unique().tolist())
    activity_map = {activity: idx for idx, activity in enumerate(activity_values)}
    ...
    if len(X_train) != len(y_train):
        raise ValueError("Training feature rows do not match the training labels.")
    if len(X_test) != len(y_test):
        raise ValueError("Test feature rows do not match the test labels.")

    metadata = {
        "risk_class_mapping": {key: int(value) for key, value in RISK_CLASS_MAPPING.items()},
...
def train_model(X_train: pd.DataFrame, y_train: np.ndarray) -> XGBClassifier:
    ...
    if X_train.empty:
        raise ValueError("Risk-model training data is empty.")

    class_counts = np.bincount(y_train)
    sample_weights = np.ones(len(y_train), dtype=float)
    num_classes = len(RISK_CLASS_MAPPING)
    for class_index, count in enumerate(class_counts):
        if count > 0:
            sample_weights[y_train == class_index] = len(y_train) / (num_classes * count)

    model = XGBClassifier(
        objective="multi:softprob",
        num_class=len(RISK_CLASS_MAPPING),
        n_estimators=300,
        learning_rate=0.05,
        max_depth=6,
        subsample=0.9,
...
def evaluate_model(
    ...
    """Evaluate the risk model using accuracy, precision, recall, F1, confusion matrix, and report."""
    predictions = model.predict(X_test)
    accuracy = accuracy_score(y_test, predictions)
    precision = precision_score(y_test, predictions, average="macro", zero_division=0)
    recall = recall_score(y_test, predictions, average="macro", zero_division=0)
    f1 = f1_score(y_test, predictions, average="macro", zero_division=0)
    confusion = confusion_matrix(y_test, predictions, labels=[0, 1, 2, 3])
    ...
    metrics = {
        ...
        "per_class_recall": {
            "LOW": float(per_class_recall[0]),
            "MEDIUM": float(per_class_recall[1]),
            "HIGH": float(per_class_recall[2]),
            "CRITICAL": float(per_class_recall[3]),
        },
...
def save_artifacts(
    ...
    """Persist the trained model, feature store, metadata, and evaluation outputs."""
    RISK_MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)
    with RISK_MODEL_PATH.open("wb") as model_file:
        pickle.dump(model, model_file)

    RISK_FEATURES_PATH.parent.mkdir(parents=True, exist_ok=True)
    features_payload = {
        "feature_names": FEATURE_NAMES,
        "rows": feature_frame.to_dict(orient="records"),
        "labels": labels.astype(int).tolist(),
    }
    with RISK_FEATURES_PATH.open("w", encoding="utf-8") as feature_file:
        json.dump(features_payload, feature_file, indent=2)

    RISK_METADATA_PATH.parent.mkdir(parents=True, exist_ok=True)
    with RISK_METADATA_PATH.open("w", encoding="utf-8") as metadata_file:
        json.dump(metadata, metadata_file, indent=2)

    RISK_RESULTS_PATH.parent.mkdir(parents=True, exist_ok=True)
    with RISK_RESULTS_PATH.open("w", encoding="utf-8") as results_file:
        json.dump(results, results_file, indent=2)


def main() -> None:
    """Train and evaluate the XGBoost Silambu risk model and save all required artifacts."""
    print("Evaluation is based on synthetic data.")

    X_train, y_train, X_test, y_test, metadata = prepare_data()
    model = train_model(X_train, y_train)
    predictions, metrics = evaluate_model(model, X_test, y_test)

    train_distribution = {
        "LOW": int(np.sum(y_train == 0)),
        "MEDIUM": int(np.sum(y_train == 1)),
        "HIGH": int(np.sum(y_train == 2)),
        "CRITICAL": int(np.sum(y_train == 3)),
    }
    test_distribution = {
        "LOW": int(np.sum(y_test == 0)),
        "MEDIUM": int(np.sum(y_test == 1)),
        "HIGH": int(np.sum(y_test == 2)),
        "CRITICAL": int(np.sum(y_test == 3)),
    }
    ...
    print(f"Class distribution: {test_distribution}")
    print(f"Test accuracy: {metrics['accuracy']:.4f}")
    print(f"LOW recall: {metrics['per_class_recall']['LOW']:.4f}")
    print(f"MEDIUM recall: {metrics['per_class_recall']['MEDIUM']:.4f}")
    print(f"HIGH recall: {metrics['per_class_recall']['HIGH']:.4f}")
    print(f"CRITICAL recall: {metrics['per_class_recall']['CRITICAL']:.4f}")
    print("Confusion matrix:")
    print(np.array(metrics["confusion_matrix"]))
    print("Classification report:")
    print(metrics["classification_report"])

    save_artifacts(model, X_test, y_test, results, metadata)
    print(f"Saved risk model to: {RISK_MODEL_PATH}")
    print(f"Saved risk results to: {RISK_RESULTS_PATH}")
    print(f"Saved risk features to: {RISK_FEATURES_PATH}")
    print(f"Saved risk metadata to: {RISK_METADATA_PATH}")
